...
In conclusion, the LASSO model was the best in predicting chances of admission with the lowest MSE of 0.0036848, and the worst being the MARS model with an MSE of 0.0038899. However, all of the models performed very similarly, so we would not consider any model's predictions significantly better than another. Because of this, we would recommend using any of the three linear models for ease of interpretation in addition to prediction accuracy. We were expecting a non-linear model to perform the best and were surprised that the LASSO model beat out the other non-linear models, albeit not by much. It turns out the non-linear models were not necessarily needed to capture the slight non-linear relationships in our data. As stated above, CGPA and GRE were the most influential predictors in our ordinary linear regression and LASSO models, with the coefficients being 0.0717 and 0.0209 respectively in our LASSO model. ALthough the coefficient for research was 0.0235, we did not consider it an influence because research is a binary outcome. Our findings make sense in that we would expect GPA and GRE scores to be the most efficient mechanism for universities to weed out large numbers of applicants, with the other factors playing a bigger role in later stages when GPA and GRE scores are more similar across considered applicants. A follow up analysis could be done to determine whether the other predictors have a bigger impact on chances of admission when all applicants have similar, strong GPA and GRE scores. We hope that our models will give graduate school applicants a better idea of what to expect in terms of chances of admission given the components of their application.
